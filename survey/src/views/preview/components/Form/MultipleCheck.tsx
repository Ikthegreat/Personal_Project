...
function MultipleCheck({ formIndex }: FormProps) {
  ...
  const handleSelect = (index: number) => {
    ...
    updatedSurvey[surveyKey].form[formIndex].context[index].isChecked =
      !updatedSurvey[surveyKey].form[formIndex].context[index].isChecked;
        ...
        (context, index) =>
          typeof context !== 'string' && (
            <Pressable style={styles.row} key={index} onPress={() => handleSelect(index)}>
              <SelectBox type="check" isSelected={context.isChecked}></SelectBox>
              <Space dir="col" size={10} />
              <Text style={styles.text}>{context.text}</Text>
            </Pressable>
          ),
      )}
      <Space dir="row" size={20} />
